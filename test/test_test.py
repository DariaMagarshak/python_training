...
def test_add_contact_to_group(app, orm, adding_contact, gr_id, c_id, current_group):
...
def find_adding_contact (app, orm):
    if len(app.contact.get_contact_list()) == 0:
        if len(app.group.get_group_list()) == 0:
            app.group.create(Group(name="test"))
        app.contact.create(Contact(firstname="test"))
    ...
    return list(adding_contact, gr_id, c_id, current_group)
